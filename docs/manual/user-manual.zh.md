...
延迟与请求速率同时升高，还发现数据库可用性指标在 10:07:00 塌到 0。日志发现 184 条数据库
连接超时。拓扑确认 `order-api` 是最早异常的服务。知识匹配到两篇 runbook。
...
领先者是**错的**，而它错在哪里值得说清楚：它并不是靠某个空缺取胜的。它所声明的每一条
要求——可用性信号塌陷、日志中的数据库连接错误——都被手上真实存在的证据满足。数据库在
10:07 确实宕了。在这批证据上，认真的人给出的就是这个答案。

**质疑者介入。** 连接池解释需要一个没人查过的饱和度指标。它还需要一次配置变更，而默认
的变更查询只回看到告警时刻。此外，目前采集到的证据没有把任何对手解释与领先者区分开。
...
服务，错误率仅 0.20%——这构成了针对流量解释的反证。而时长对比显示：数据库从 10:09:00 起
已经恢复可用，错误率却一直高到 10:27:00——多出来的十八分钟失败，两分钟的宕机解释不了。
...
| 2 | 数据库不可用 | 0.47 | 宕机比错误早结束十八分钟 |
| 3 | 流量上涨超出容量 | 0.29 | 携带反证，被惩罚 0.20 |

两个对手解释最终都是被**反证驳倒**的，而不只是分数上被压过：各自都带着一条针对它记录在案
的证据——这比在分数上落败是更强的结论。
...
这组样例中有三个在做特定的事。`C4` 的正确答案恰恰是 `sig-traffic-surge`——也就是参考场景
用整个第 2 轮去降级的那个解释——两个基线都答对了它。它之所以存在，就是为了让一个仅仅学到
"领先的解释就是错的"的系统必然在某处失败；也正因如此，对抗流程的 100% 才是一个关于**区分
能力**的主张，而不是关于条件反射式反对的主张。`C5` 的告警落在故障的下游服务上，因此答案
根本不在发出告警的那个服务里。`C6` 里有一次配置变更与领先解释完美吻合，却发生在错误开始
**之后**四分钟；唯一能反驳它的就是先后顺序。

看这些数字时请带上两条注意，报告也会把它们打印在旁边。第一，样本是六个故障样例；六个样例
上的 100% top-1 准确率，只是关于这六个样例的陈述。第二，单 Agent 基线拿到的是与完整流程
...
这个适配器已经实现。它可以对接任何 chat-completions 风格的 JSON API：
...
API key 从 `ARENA_MODEL_API_KEY` 读取。刻意不设默认端点：本项目尚未选定服务商，而设一个
默认值等于替你做了一个这段代码无权替你做的决定。

**模型被要求做什么，以及不做什么。** 它选择目录中的哪些签名被证据支持，以及各自由哪些
证据支持。它不发明解释，也不给出分数——分数由规则引擎所用的同一个打分器、同一套权重在本地
计算。这一划分背后的理由见
[ADR-007](../adr/007-model-selects-from-catalog.zh.md)；简言之：模型断言的数字既拆不开、
也无法被反驳、更无法据以行动，而这个系统三者都需要。

因此模型推理器无法命名一个不在目录中的原因。这是一条真实的限制，在此明说而非藏起来：
新增一个解释意味着新增一条签名，而那是一次数据变更。

**当服务商发生故障时**，适配器返回带类型的错误，而不是空结果。空假设列表意味着"没有任何
匹配"；一个已经挂掉的服务商，不得有资格说出这句话。
