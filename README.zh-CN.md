...
第 1 轮 —— 五个采集器，默认查询
  ...
  数据库在 10:07 确实掉线了，日志里也全是连接超时。宕机这个解释所声明的每一条
  要求，都被手上已有的证据满足——在这批证据上，认真的人给出的就是这个答案。

  质疑者：目前没有任何证据把这些对手解释区分开。连接池解释需要一个没人查过的
  饱和度指标，以及一次没人回看得够远因而没找到的变更。
  → 发出四条索证，调查返回采集阶段
...
第 2 轮 —— 索证被满足
  ...
  宕机持续了两分钟，错误却持续了二十分钟。可用性在 10:09 就恢复了，而 5xx
  一直高到 10:27——所以这些错误无论是什么造成的，都不是那次宕机。

  → set_config order-api DB_POOL_SIZE 20，中风险，停在审批门
  → 批准之后：5xx 0.129 → 0.003，延迟 1623ms → 190ms，已恢复
```

质疑者不是给结果加了批注，而是**改变**了结果——而且它推翻的不是一处疏漏，
而是一个完整、有充分证据支撑的错误答案。
...
连基线的 top-3 里都没有正确答案的样例——这正是它们覆盖率为 83% 而非 100% 的原因。

两条注意事项，会与数字一同打印而不是被藏起来：样本是六个故障样例；单 Agent 基线拿到的是与
