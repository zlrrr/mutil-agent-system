...
基线并非一无是处：它们在 `C4` 上是对的。那个样例的正确答案恰恰是 `sig-traffic-surge`——
也就是参考场景用整个第 2 轮去降级的那个解释。它之所以存在于目录中，正是为了让一个学到
"领先的解释就是错的"的系统必然在某处失败；也正因如此，对抗流程的 100% 才意味着它在**区分**，
而不是在条件反射地反对。

两条注意事项，会与数字一同打印而不是被藏起来：样本是四个故障样例；单 Agent 基线拿到的是与
