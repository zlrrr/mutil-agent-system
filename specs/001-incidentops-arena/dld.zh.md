...
`Leading()` 返回质疑者未曾拒绝的、分数最高的那个假设；`Snapshot.Leading()` 与之保持一致，
使 Agent 与控制平面对"哪个解释才是本 case 的答案"取得共识。分数衡量证据吻合得多好，裁决
衡量该解释是否可被采纳。一个解释完全可能吻合得极好却根本不可能成立；因此，在一份同时记录了
它被拒绝的报告里，把排名最高的那个当作答案呈现，会让系统自相矛盾。排序本身保持不动：它依然
把被拒绝的解释排在第一，旁边并列其裁决与反证。

被跳过的只有 `reject`。`revise` 表示质疑者还想再做些工作，而不是该解释不可采纳；连它一起跳过，
会让报告给出的答案取决于"一条尚未完成的反对意见有多严厉"，而不是取决于证据：曾有一个 case，
最好的解释得分 0.84 却带着 `revise`，于是它转而报告了一个 0.09 的解释——只因为后者招致的质疑
更温和。一个解释**可否被执行**是另一个问题，由 `CanRemediate`（DLD-1062）单独回答，而那里
确实要求裁决是允许推进的。
...
索证响应可以指定 `Service` 与 `Logs` 关键词。`Service` 默认为告警的服务——这在告警服务确实
是主体时是对的；可一旦它其实是受害者，能了断此案的变更历史与日志行就属于它上游的某个服务，
而一个按告警服务限定的响应将什么也返回不了。指定了服务的响应会把它记录在 `subject` 事实
中，后续规则正是据此判断某个解释是否建立在上游证据之上（DLD-1034）。
...
| `alternative_explanation` | 另一条签名至少匹配一个模式、不携带未消解的反证，且它有某条区分性证据既未被回答、也未被它自身已匹配的要求所提供 | 对领先假设发出点名该对手的 `revise` 质疑，外加对区分性证据的索证 |
| `temporal_order` | 该假设自身的证据晚于告警超过 `coMovementWindow`，或它匹配到的变更时间戳不早于异常起点 | `reject` 质疑，并向该假设附加反证 |
| `source_vs_victim` | 拓扑证据指出某上游服务起点更早，**且**该假设不依赖任何 `subject` 事实为该服务的证据 | 点名该上游候选的 `revise` 质疑，外加针对目录中每条尚未被回答的要求描述符的索证 |
...
**为什么 `temporal_order` 要检查两件事。** 变更顺序那一项是拿被归咎的变更与
`HypothesisOnset` 比较，而后者衡量的是解释与它**自身**支持指标的关系。正是这一点让真正的
原因能在"某条无关序列动得更早"时存活下来（DLD-1032）——但它同样是个漏洞：一个先被调小、
随后饱和的连接池在内部是自洽的，却对一场早四分钟就开始的故障什么也没解释。第一个条件通过
...
**为什么 `alternative_explanation` 到了预算边界就停手。** 当已经没有下一轮时，索证永远不可能
被回答，于是承载它的那条挑战也永远无法解除——案件会因为一个"它根本没机会去问"的问题，而无法
对自己最好的解释采取行动。此时规则改为保持沉默；未被检验的部分以"未满足索证"的形式记录在案，
而真正的分差过小仍由专门处理它的那条规则升级。

**为什么 `source_vs_victim` 按已有支撑度排序它的索证。** 它索要的是整个目录中尚未被回答的
要求描述符，因此每新增一条签名，它的索证清单就变长一分，而 `maxDemandsPerRound` 随后决定
调查实际会去追哪几条。按签名标识符排序，等于把这个决定交给字母顺序的偶然：新增两条签名，就
把某个受害者样例真正需要的证据挤了出去，于是它再也没能抵达自己的原因。现在已被证据部分支撑
的签名排在前面——那些是线索，其余的是碰运气。

**为什么 `alternative_explanation` 还有另外两个跳过。** 这条规则的存在，是为了阻止"在还有一个
同样自洽的对手未被检验时就接受领先者"。因此两个跳过问的是同一个问题：这个对手真的还未被
检验吗？

*证据已在手。* `demandAlreadyAnswered` 只认得"作为索证的回答而到达"的证据，因此案件按默认
查询采集到的证据对它是不可见的。于是，一个区分性要求已被自身匹配所满足的对手仍会被再索一
次，白白花掉一整轮采集去重取第一轮就读过的序列。质疑者手上有没有那份区分性证据，不应取决
于它是怎么拿到的。

*已被反证驳过。* 一个已被采集到的证据所反对的对手——即携带未消解反证的对手——已经被检验过，
并且已经失分。继续索要那份"用来区分它"的证据，是在要求案件重做已经做过的工作。更糟的是，
当没有任何来源能提供该证据时，这条索证会在每一轮被重新发出，领先者于是永远无法被接受：案件
烧光轮次预算，并把一个证据其实早已回答了的问题升级给人。

**为什么 `source_vs_victim` 同时带有跳过与索证。** 没有跳过，它会连那个"本就归咎于上游"
的解释一起挑战，用的还正是支撑该解释的那份拓扑证据。没有索证，它说完"你可能看错了服务"
就什么也不要，于是调查带着一个 `revise` 裁决直接收尾，而不是被重定向——一个没有任何证据
能够回应的挑战，是否决，而不是质疑（REQ-0101）。

这个跳过是由**证据**判定的，而不是由签名判定的。签名的处置字段按目录惯例写死了服务名而非
经由推断得出——本目录中每条签名的处置都指向 `order-api`——因此若从该字段读取意图，会让每个
解释都显得"已经看向上游"。回应索证的证据会在 `subject` 事实中记录它所涉及的服务，规则读的
正是这一项。
...
（120 → 260 rps，起点 10:03）、`db_up`（基线 1，10:07 塌到 0，10:09 恢复）、日志簇
`db connection timeout after #ms`（184 行）、指明 `order-api` 为候选源头的拓扑，以及一条
runbook 匹配。变更采集器的默认趟只覆盖故障窗口，因此 10:05:30 的那次发布**没有**被发现。
...
领先假设是**错的**，而它错成什么样，正是这个场景的要害。它并不是靠某个空缺领先的：
`sig-db-outage` 要求可用性信号塌陷，以及日志中出现数据库连接错误，而第 1 轮两者都有。
它所声明的每条要求都被手上的证据满足，metric 与 log 两项都是 1.0，并且领先第二名 0.16
——在 `closeCallMargin` 之外，因此这是一个**有把握**的答案，而不是一次掷硬币。在这批证据
上，认真的推理者给出的就是它，而它依然是错的。

领先者仍低于 `acceptThreshold`。`alternative_explanation`、`coverage_gap` 与
`unverifiable_remediation` 触发，产生对连接池饱和度指标、`changeLookback` 范围内的配置
变更历史、错误率持续时长与数据库恢复时刻的对比，以及等负载历史峰值对比的索证。数据库
可用性指标**没有**被索要：`sig-db-outage` 自身已匹配的要求就提供了它，而索要案件已经握有
的证据，只会花掉一轮去取回第 1 轮已经读过的东西。

**第 2 轮 — 索证驱动的查询。** `db_pool_saturation` 返回饱和（容量 1.0，峰值 1.0，起点
10:06）；扩展窗口内的变更查询返回 10:05:30 的 `DB_POOL_SIZE 20 → 2`；流量对比返回两天前
的等负载时段且当时无错误，并携带 `Facts["counters"] = "sig-traffic-surge"`；时长对比报告
可用性在 10:09:00 已恢复而错误持续到 10:27:00，并携带
...
排序发生了变化，且首位现在是正确的。请注意 `sig-db-outage` 身上**没有**发生什么：它的
metric 与 log 两项分毫未变，因为那次宕机是真的，支撑它的证据依然成立。把它挤下第一名的是
反证惩罚——它是被**驳倒**的，不是被"测"输的。时序成立（10:05:30 < 10:06），全部索证已满足，
覆盖四个类别，总分越过阈值——因此裁决为 `accept`，case 推进到处置阶段，动作为
`set_config order-api DB_POOL_SIZE 20`，风险 `medium`，回滚到 `2`，验证
`http_5xx_rate` 与 `http_request_duration_p99`。

**这演示了什么。** 单 Agent 模式与无质疑模式都停在第 1 轮，报告"数据库宕机"——那个第一个
自洽、且证据齐备的故事。只有对抗流程会去问"那次宕机持续了多久"，并因此抵达配置变更。这正是
评估所计算的对比，而且它是流程的性质而非数据的性质，因为三种模式看到的是同一份夹具。它同时
也比它所替换的那个版本更难：一个仅仅"缺证据"的首轮错误，会让质疑者靠补上空缺就取胜，而那对
"对抗式复核"说明不了多少——推翻一个要什么有什么的答案才说明得了。
