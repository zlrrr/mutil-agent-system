...
**What the ensemble must be strong enough to do.** The bar is not "find the requirement
nobody collected" (REQ-0103). A rule ensemble that only detected coverage gaps would pass
a scenario whose first answer was incomplete, and would have nothing to say about one
that was complete and wrong. So the demand power and the veto power are not
interchangeable: coverage gaps are answered by *collecting*, whereas a fully evidenced
wrong answer is displaced only by evidence recorded against it. Both paths must exist,
