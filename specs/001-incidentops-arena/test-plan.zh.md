...
正是这个样例让评估保持诚实。C1 结束时 `sig-traffic-surge` 被反证驳倒并排在末位，因此
...
#### TC-0110 — 首轮错误是"证据齐备"的，而不只是"无人反对"

**层级。** e2e。**验证。** REQ-0103。

**步骤。** 以一轮的预算分别运行 `C1`、`C2`、`C3`，让每个 case 恰好停在第 1 轮结束处；随后
各自再完整运行一次。

**预期。** 第 1 轮结束时，每个 case 的领先者都是下表中那个错误解释；把它的签名与已采集证据
做匹配，**没有**任何未匹配的要求；并且它对第二名的分差大于 `closeCallMargin`。完整运行之后，
被接受的原因是表中正确的那一个，而首轮领先者仍留在排序中，并携带一条记录了表中所列事实的
反证。

| 样例 | 首轮领先者（错误） | 被接受（正确） | 靠什么反驳它 |
|---|---|---|---|
| C1 | `sig-db-outage` | `sig-db-pool-exhaustion` | 宕机比错误早结束十八分钟 |
| C2 | `sig-upstream-outage` | `sig-misconfigured-dependency` | 健康检查恰恰是透过那个配错的端点抓取的 |
| C3 | `sig-cold-cache` | `sig-slow-query` | 十二个端点里只有一个变慢，而它们共用同一个存储 |

这个用例之所以存在，是因为这些场景此前恰恰**没有**测到这一点。C1 的首轮错误是靠"默认"取胜
的——能击败它的证据当时还没被采集，因此质疑者只需要补上一个空缺。C2 与 C3 更糟：它们的首轮
领先者是那个匹配了"根本没动过的请求速率"的流量解释，而正确答案分别只落后 0.01 与 0.03。
纠正一次掷硬币，什么也证明不了。

这三种反驳在**种类**上是刻意不同的——一个结束得太早的故事、一个透过坏掉的东西去测量的故事、
一个窄到配不上自己所声称之物的故事——因为如果目录中每个错误答案都倒在同一种检查之下，那证明
的是那一种检查，而不是对抗式复核。每个 case 的两条断言也是刻意分开的：一个证据齐备但只领先
0.02 的领先者等于掷硬币；而一个遥遥领先却缺着要求的领先者，就又回到了旧场景。
