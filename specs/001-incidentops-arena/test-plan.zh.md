...
**步骤。** 以一轮的预算运行 `C1`，这会让 case 恰好停在第 1 轮结束处；随后再完整运行一次。

**预期。** 第 1 轮结束时领先者为 `sig-db-outage`；把该签名与已采集证据做匹配，**没有**任何
未匹配的要求；并且领先者对第二名的分差大于 `closeCallMargin`。完整运行之后，被接受的原因是
`sig-db-pool-exhaustion`，而 `sig-db-outage` 仍留在排序中，并携带一条记录"错误比数据库多
持续了多久"的反证。

这个用例之所以存在，是因为旧的 C1 恰恰**没有**测到这一点。它的首轮错误是靠"默认"取胜的：
能击败它的证据当时还没被采集，因此质疑者只需要补上一个空缺。现在这个场景里包含一次与故障
同时开始的、真实的两分钟数据库宕机，于是第 1 轮会得到一个"所声明的每条要求都被满足"的解释
——第 2 轮必须把它拆掉，而不是把它补全。两条断言是刻意分开的：一个证据齐备但只领先 0.02 的
领先者，等于掷硬币；而一个遥遥领先却缺着要求的领先者，就又回到了旧场景。
