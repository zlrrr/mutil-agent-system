...
```go
package planmodel
func New(endpoint, model string, opts Options) *Planner // 实现 plan.Planner
```

它拿到的是告警、可用序列名与轮次编号，被问的是"查哪些、为什么"。它**从不**被授予对时间窗的
自由裁量：triage 的窗口来自告警与声明的回看区间——一个能把窗口放宽的策略，也就能让每个 case
变得无界。
